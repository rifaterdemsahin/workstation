...
if %errorlevel% equ 0 (
    ...
    powershell -ExecutionPolicy Bypass -File "%~dp0launch_telegram_gemini.ps1"
) else (
    echo PowerShell not available, trying AutoHotkey...
    if exist "%~dp0launch_telegram_gemini.ahk" (
        if exist "C:\Program Files\AutoHotkey\AutoHotkey.exe" (
            "C:\Program Files\AutoHotkey\AutoHotkey.exe" "%~dp0launch_telegram_gemini.ahk"
        ) else if exist "C:\Program Files (x86)\AutoHotkey\AutoHotkey.exe" (
            "C:\Program Files (x86)\AutoHotkey\AutoHotkey.exe" "%~dp0launch_telegram_gemini.ahk"
        ) else (
