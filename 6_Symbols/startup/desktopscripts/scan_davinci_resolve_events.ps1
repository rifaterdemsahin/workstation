...
# Never let an unhandled error exit the script silently
...
Function Get-RelevantEvents {
    ...
    try {
        ...
        foreach ($ev in $raw) {
            $src      = if ($ev.Source)  { $ev.Source }  else { "" }
        ...
        Write-Host "  [WARN] Could not read '$LogName' log: $_" -ForegroundColor Yellow
    }
    return $results
}

# ── Everything runs inside try/finally so the close dialog ALWAYS appears ─────
$fatalError = $null
...
try {
    ...
    # ── Build plain-text report and auto-copy to clipboard ───────────────────
    ...
    $fatalError = $_
    Write-Host "`n  [ERROR] Script encountered a fatal error:" -ForegroundColor Red
    Write-Host "  $fatalError" -ForegroundColor Red
}
finally {
    # ── Always block here — never closes without user clicking OK ─────────────
